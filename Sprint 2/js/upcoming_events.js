...
let n = futureEvents.length;
for (let i = 0; i < n - 1; i++) {
    for (let j = 0; j < n - i - 1; j++) {
        if (new Date(futureEvents[j]['date']) > new Date(futureEvents[j + 1]['date'])) {
            [futureEvents[j], futureEvents[j + 1]] = [futureEvents[j + 1], futureEvents[j]];
        }
    }
}
...
function carouselSlider(event, isActive) {
    ...
    h5.textContent = "UPCOMING EVENTS";
...
// Función para crear la card del evento
function eventCard(event) {
    ...
    cardLink.href = "/Sprint 2/pages/details.html";
